...
| `phone` | `MNGR_TELNO` | 일부 마스킹된 원본 그대로. **화면에는 표시하지 않는다** |

`MNGR_NM`·`MKR_NM`·`MDL_NM`·`CTPV_NM`·`SE`는 화면에서 쓰지 않아 스냅샷에 넣지 않는다.

**`phone`을 화면에 안 쓰는 이유** — 원본이 이미 가려져 와서 걸 수 있는 번호가 아니다.
스냅샷 61,717건 실측 결과 **99.9%(61,686건)가 `*`를 포함**하고, 나머지 25건도 `055`,
`112` 같은 조각이라 쓸 수 없다. 마스킹은 공공데이터포털이 개인정보 보호로 적용하는 것이며
`sync.py`는 값을 가공하지 않는다 — **코드로 되살릴 방법이 없다.**
필드 자체는 스냅샷에 남겨둔다. 지우려면 전량 재생성으로 10MB 커밋이 한 번 더 생기는데
압축 후 절약은 100KB 수준이고, 정책이 바뀌면 코드 수정 없이 되살릴 수 있기 때문이다.
