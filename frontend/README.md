...
| `search.js` | `Search` — 기관명·주소 검색 (저장된 데이터만 훑어 오프라인에서도 동작) |
...
**검색은 지오코딩 API가 아니라 저장된 데이터를 훑는다** — 외부 API를 붙이면 오프라인에서
검색이 죽는다. 이미 메모리에 있는 61,717건의 기관명·주소·설치위치를 부분일치로 찾는다.
대가는 "AED가 없는 임의 주소는 못 찾는다"는 것인데, AED를 찾는 게 목적이므로
AED 레코드를 검색하는 편이 자연스럽다. PC는 GPS가 없어 위치를 못 잡는 일이 흔한데
그때 이 검색이 유일한 진입 경로가 된다.

두 가지가 실제로 깨져서 고쳤다. **① 상한(20개)에 걸리면 스캔을 멈추던 것** —
스냅샷이 좌표순(남→북)이라 서울 사용자가 "중구"를 쳐도 부산 중구만 나왔다.
전량을 훑고 기준점에서 가까운 순으로 정렬한다(위치를 모르면 기관명 매칭을 주소보다 앞세운다).
**② 주소로 중복을 제거하던 것** — `address_road`에 층 표기까지 들어 있어 같은 건물의
'서울갤러리'가 두 줄로 나왔다. 기관명 + 좌표 3자리(약 100m)로 묶는다.

**검색 지점은 "내 위치"로 표시하지 않는다** — 지도에서는 속이 빈 회색 점(`.search-dot`),
간이 지도에서는 회색 링으로 그린다. 내가 있지도 않은 곳을 파란 "내 위치"로 보여주면
사용자를 속이는 것이다.
